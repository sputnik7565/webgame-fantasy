// events.js
function nextTurn() {
    if (Math.random() < 0.7) {  // 몬스터와 조우할 확률 70%
...
function createMonsters(playerLevel) {
    function getRandomLevel(baseLevel) {
        return Math.max(baseLevel + Math.floor(Math.random() * 3) - 1, 1); // 최소 레벨 1 보장
    }
    ...
    return [
        new Monster("고블린", getRandomStat(30 + 5 * playerLevel, 10), getRandomStat(10 + 2 * playerLevel, 4), getRandomStat(1 + Math.floor(playerLevel / 2), 1), getRandomLevel(playerLevel), "고블린이 으르렁거린다!", paths.goblinImage),
        new Monster("오크", getRandomStat(50 + 10 * playerLevel, 20), getRandomStat(15 + 3 * playerLevel, 6), getRandomStat(3 + Math.floor(playerLevel / 2), 2), getRandomLevel(playerLevel), "오크가 분노한다!", paths.orcImage),
        new Monster("트롤", getRandomStat(80 + 15 * playerLevel, 30), getRandomStat(20 + 4 * playerLevel, 8), getRandomStat(5 + Math.floor(playerLevel / 2), 3), getRandomLevel(playerLevel), "트롤이 거대하게 다가온다!", paths.trollImage),
        new Monster("드래곤", getRandomStat(150 + 20 * playerLevel, 40), getRandomStat(25 + 5 * playerLevel, 10), getRandomStat(10 + Math.floor(playerLevel / 2), 5), getRandomLevel(playerLevel), "드래곤이 불을 뿜는다!", paths.dragonImage),
        new Monster("뱀파이어", getRandomStat(60 + 12 * playerLevel, 24), getRandomStat(18 + 4 * playerLevel, 7), getRandomStat(4 + Math.floor(playerLevel / 2), 2), getRandomLevel(playerLevel), "뱀파이어가 웃으며 다가온다!", paths.vampireImage),
        new Monster("웨어울프", getRandomStat(70 + 14 * playerLevel, 28), getRandomStat(20 + 5 * playerLevel, 8), getRandomStat(6 + Math.floor(playerLevel / 2), 3), getRandomLevel(playerLevel), "웨어울프가 포효한다!", paths.werewolfImage),
        new Monster("해골", getRandomStat(20 + 4 * playerLevel, 8), getRandomStat(8 + 2 * playerLevel, 3), getRandomStat(1 + Math.floor(playerLevel / 2), 1), getRandomLevel(playerLevel), "해골이 덜그럭거린다!", paths.skeletonImage),
        new Monster("좀비", getRandomStat(40 + 8 * playerLevel, 16), getRandomStat(12 + 3 * playerLevel, 5), getRandomStat(2 + Math.floor(playerLevel / 2), 1), getRandomLevel(playerLevel), "좀비가 비틀거리며 다가온다!", paths.zombieImage),
        new Monster("악마", getRandomStat(100 + 20 * playerLevel, 40), getRandomStat(22 + 6 * playerLevel, 9), getRandomStat(8 + Math.floor(playerLevel / 2), 4), getRandomLevel(playerLevel), "악마가 불길하게 웃는다!", paths.demonImage),
        new Monster("유령", getRandomStat(50 + 10 * playerLevel, 20), getRandomStat(16 + 3 * playerLevel, 6), getRandomStat(3 + Math.floor(playerLevel / 2), 2), getRandomLevel(playerLevel), "유령이 어두운 속삭임을 한다!", paths.ghostImage)
    ];
}
